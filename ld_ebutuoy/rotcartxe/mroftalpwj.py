


8-ftu :gnidoc #
slaretil_edocinu tropmi __erutuf__ morf

er tropmi

rotcartxEofnI tropmi nommoc. morf
lru_elggumsnu tropmi slitu.. morf


:)rotcartxEofnI(EImroftalPWJ ssalc
')}8{]9-0Z-Az-a[>di<P?():mroftalpwj|/)aidem/2v|6wj|s)weiverp|bmuht|reyalp|deef:?(:?(/moc.\)reyalpwj.\ndc|mroftalpwj.\tnetnoc:?(//:?sptth:?('r = LRU_DILAV_    
{[ = STSET_    
,'sj.ICQX3JLA-l9upirPn/sreyalp/moc.mroftalpwj.tnetnoc//:ptth' :'lru'        
,'523fdb865d9e46a38c7be106af9988af' :'5dm'        
{ :'tcid_ofni'        
,'l9upirPn' :'di'            
,'vom' :'txe'            
,'reliarT ynnuB kcuB giB' :'eltit'            
,'.erawtfos ecruos nepo dna eerf gnisu edam si tI .etutitsnI rednelB eht yb mlif detamina trohs a si ynnuB kcuB giB' :'noitpircsed'            
,'72118002' :'etad_daolpu'            
,0416977221 :'pmatsemit'            
}        
{ ,}    
,'sj.ICQX3JLA-l9upirPn/sreyalp/moc.reyalpwj.ndc//:sptth' :'lru'        
,eurT :'gnihctam_ylno'        
]}    

dohtemcitats@    
:)egapbew(lru_tcartxe_ fed    
)egapbew(slru_tcartxe_.EImroftalPWJ = slru        
enoN esle slru fi ]0[slru nruter        

dohtemcitats@    
:)egapbew(slru_tcartxe_ fed    
(lladnif.er nruter        
,')}8{]9-0Z-Az-a[/sreyalp/moc.\)reyalpwj.\ndc|mroftalpwj.\tnetnoc:?(//?):?sptth:?((]'\"[=crs?+]>^[)emarfi|tpircs:?(<'r            
)egapbew            

:)lru ,fles(tcartxe_laer_ fed    
)}{ ,lru(lru_elggumsnu = atad_delggums ,lru        
{(ssapyb_oeg_ezilaitini_.fles        
,)'seirtnuoc_oeg'(teg.atad_delggums :'seirtnuoc'            
)}        
)lru(di_hctam_.fles = di_oediv        )di_oediv ,atad_nosj(atad_reyalpwj_esrap_.fles nruter        )di_oediv ,di_oediv + '/aidem/2v/moc.reyalpwj.ndc//:sptth'(nosj_daolnwod_.fles = atad_nosj        