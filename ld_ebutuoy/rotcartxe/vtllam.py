


8-ftu :gnidoc #
slaretil_edocinu tropmi __erutuf__ morf

er tropmi

rotcartxEofnI tropmi nommoc. morf
stcid_egrem tropmi slitu.. morf


:)rotcartxEofnI(EIVTllaM ssalc
')+]&#?/^[>di<P?(*)/+]/^[:?(/vt.\llam?).\)ks|www:?(:?(//:?sptth'r = LRU_DILAV_    
{[ = STSET_    
,'ecivajip-insit-v-kevolc-oben-icvotrops-uosj-udvarpo-ykvoksizen-orp-drailim-81/vt.llam.www//:sptth' :'lru'        
,'815e85434b7a3013203f1e080f73a4c1' :'5dm'        
{ :'tcid_ofni'        
,'0tzz0t' :'di'            
,'ecivajip-insit-v-kevolc-oben-icvotrops-uosj-udvarpo-ykvoksizen-orp-drailim-81' :'di_yalpsid'            
,'4pm' :'txe'            
,'?ecivajip ins­Ãt v k›ÄvolŒÄ oben icvotrops uosj udvarpO .ykvoksizen orp drailim 81' :'eltit'            
,'bed92af386c206b206ab27a24ce0cf52:5dm' :'noitpircsed'            
,612 :'noitarud'            
,0040788351 :'pmatsemit'            
,'70018102' :'etad_daolpu'            
,tni :'tnuoc_weiv'            
}        
{ ,}    
,'ecivajip-insit-v-kevolc-oben-icvotrops-uosj-udvarpo-ykvoksizen-orp-drailim-81/italp-ot-odk/vt.llam.www//:sptth' :'lru'        
,eurT :'gnihctam_ylno'        
{ ,}    
,'akcakeps-obela-alpet-kinboryv-ejerhen-aicamalker/suahmjeg/vt.llam.ks//:sptth' :'lru'        
,eurT :'gnihctam_ylno'        
]}    

:)lru ,fles(tcartxe_laer_ fed    
)lru(di_hctam_.fles = di_yalpsid        

(egapbew_daolnwod_.fles = egapbew        
))(sredaeh_noitacifirev_oeg.fles=sredaeh ,di_yalpsid ,lru            

'b\)xedni/)+]z-ad\[>di<P?(/)+]s\^[|+).)2\!?(:?()]'\"[(:?(=crsb\+]>^[ecruos<('r = ER_ECRUOS        
(xeger_hcraes_.fles = di_oediv        
)'di'=puorg ,'di oediv' ,egapbew ,ER_ECRUOS            

(seirtne_aidem_5lmth_esrap_.fles = aidem        
,di_oediv ,)egapbew ,'8u3m.1\'r ,ER_ECRUOS(bus.er ,lru            
]0[)'evitan_8u3m'=locotorp_yrtne_8u3m ,'slh'=di_8u3m            

)}{=tluafed ,di_oediv ,egapbew(dl_nosj_hcraes_.fles = ofni        

{ ,ofni ,aidem(stcid_egrem nruter        
,di_oediv :'di'            
,di_yalpsid :'di_yalpsid'            
,di_yalpsid ro )enoN=tluafed ,egapbew(eltit_hcraes_go_.fles :'eltit'            
,)enoN=tluafed ,egapbew(noitpircsed_hcraes_go_.fles :'noitpircsed'            )}        ,)enoN=tluafed ,egapbew(lianbmuht_hcraes_go_.fles :'lianbmuht'            